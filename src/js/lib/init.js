...
myImage.id = 'kitten';
myImage.src = './build/assets/img/300.jpg';
...
document.addEventListener('readystatechange', event => {
    ...
    if (event.target.readyState === "interactive") {  //same as "DOMContentLoaded" event or jQuery.ready
        // All HTML DOM elements are now accessible
    }

    if (event.target.readyState === "complete") { // same as $(window).bind("load", runFunction);
